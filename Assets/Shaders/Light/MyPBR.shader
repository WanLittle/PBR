...
// Specular镜面反射
// D项
float lerpSquareRoughness = pow(lerp(0.002, 1, roughness), 2); //把roughness lerp到了0.002，保证在smoothness为0表面完全光滑时也会留有一点点高光
...
float3 iblDiffuseResult = kdLast * iblDiffuse * Albedo;
